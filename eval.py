...
def compute_str_em(data, dataset_name):
    ...
    if dataset_name == 'ASQA':    
        ...
        for item in data:
            ...
            for ans in item['answers']:
                loc_acc.append(exact_presence([ans], item["output"]))
            acc.append(np.mean(loc_acc))
            hit.append( int(np.mean(loc_acc) == 1) )
    elif dataset_name=='TQA':
        for item in data:
            acc.append(exact_presence(item['answers'], item["output"]))
            hit.append(exact_presence(item['answers'], item["output"]))
    elif dataset_name=='NQ':
        if type(data[0]["answers"][0]) is list:
            for item in data:
                max_acc=0
                for ans_group in item['answers']:
                    loc_acc = []
                    for ans in ans_group:
                        loc_acc.append(exact_presence([ans], item["output"]))
                    acc_mean=np.mean(loc_acc)
    ...
    for item_index, item in enumerate(tqdm(data)):
        ...
        for sent_id, sent in enumerate(sents):
            ...
            if len(ref) == 0:
                # No citations
                joint_entail = 0
            elif any([ref_id >= len(item['select_docs']) for ref_id in ref]):
                ...
                joint_passage = '\n'.join([_format_document(item['select_docs'][psgs_id]) for psgs_id in ref])
            ...
            if joint_entail and len(ref) > 1:
                ...
                for psgs_id in ref:
                    # condition A
                    passage = _format_document(item['select_docs'][psgs_id]) 
                    ...
                    if not nli_result:
                        ...
                        passage = '\n'.join([_format_document(item['select_docs'][pid]) for pid in subset_exclude])
                        nli_result = _run_nli_autoais(passage, target_sent)
...
def compute_qampari_f1(data, cot=False):
    ...
    for item in data:
        ...
        answers = [[normalize_answer(x) for x in ans["aliases"]] for ans in item['answer_list']]
...
def main():
    ...
    for i in range(len(data)):
        ...
        # doc_list=data[i]["select_docs"]
        # data[i]['output']="".join([make_doc_prompt(doc, doc_id, "Document [{ID}](Title: {T}): {P}") for doc_id, doc in enumerate(doc_list)])
    ...
    result['str_em'], result['str_hit'], acc, hit = compute_str_em(normalized_data, dataset_name)
    print(result)
    if qampari:
        qampari_res, qampari_prec, qampari_rec=compute_qampari_f1(normalized_data, cot=args.cot)
        result.update(qampari_res)
        # import pdb; pdb.set_trace()
    ...
    if args.citations: 
        rec_prec, citation_rec, citation_prec = compute_autoais(data, qampari=qampari, at_most_citations=args.at_most_citations)
        result.update(rec_prec)
    if args.claims_nli:
    ...
    for index in range(len(data)):
        data[index]["correct_rec."]=acc[index]
        if args.citations: 
            data[index]["citation_rec."]=citation_rec[index]
            data[index]["citation_prec."]=citation_prec[index]
    #     if qampari:
    #         data[index]["qampari_rec."]=qampari_rec[index]
    #         data[index]["qampari_prec."]=qampari_prec[index]

    #     if acc[index]==1:
    #         if qampari and qampari_prec[index]==1:
    #             if args.citations and citation_rec[index]==1 and citation_prec[index]==1:
    #                 cnt+=1
    #             else:
    #                 cnt+=1
    #         else:
    ...
    #                 cnt+=1
    # print(cnt)
    with open(args.f[:-5]+"_score.json", "w") as f:
